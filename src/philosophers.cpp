...
std::mutex terminal;
std::shared_ptr<std::mutex[]> w;
std::shared_ptr<bool[]> ma_widelec;
...
int main(int argc, char* argv[]) {
    ...
    ma_widelec.reset(new bool[filozofowie]);
    w.reset(new std::mutex[filozofowie]);
    cv.reset(new std::condition_variable[filozofowie]);
